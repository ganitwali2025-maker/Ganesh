...
# Find the start of the return statement in GaneshChaturthiApp
match = re.search(r"(\s+const NAV = \[[\s\S]*?\];\s+)(return \([\s\S]*?\n\})", content)
if not match:
...
function TransactionItem({ item }) {
  ...
  // Actually, mockup: green arrow up for jama, purple arrow down for nikasi, blue UPI, orange users.
...
new_return = """  return (
    <div className="w-full min-h-screen flex justify-center bg-[#F4F3F8] sm:bg-gray-200 sm:py-8" style={{ fontFamily: "'Poppins','Segoe UI',system-ui,sans-serif" }}>
      ...
      <div className="w-full max-w-[412px] flex flex-col relative bg-[#F8F9FA] sm:rounded-[36px] sm:overflow-hidden sm:shadow-2xl sm:border-[8px] sm:border-gray-800" style={{ height: "100%", minHeight: "915px" }}>
        ...
        <div className="relative pb-20 pt-8 px-5 shrink-0" style={{ background: `linear-gradient(145deg, #5B21B6 0%, #7C3AED 100%)`, color: "#fff", borderBottomLeftRadius: '32px', borderBottomRightRadius: '32px' }}>
          ...
          <div className="flex items-center gap-4">
             ...
             <div>
               ...
               <div className="flex items-center gap-1.5 text-xs opacity-90 font-medium"><MapPin size={12} /> नागर्गांव / रायपुर / धर्सीवा</div>
             </div>
          </div>
        </div>
        ...
        {/* Bottom Navigation */}
        <div className="absolute bottom-0 w-full bg-white border-t border-gray-100 flex justify-around py-2.5 pb-6 shadow-[0_-4px_25px_rgba(0,0,0,0.04)] z-20 rounded-b-[36px]">
